        ...
        {loopSteps.map((step, index) => (
          <li key={step.href}>
            ...
            >
              ...
              <div className="mt-6 font-display text-[17px] font-bold tracking-[-0.02em] text-ink-900">
                <span className="sr-only">Step {index + 1}: </span>
                {step.label}
              </div>
              {step.description && (
                <div className="mt-1.5 text-[14px] leading-[1.6] text-ink-500">{step.description}</div>
              )}
            </Link>
          </li>
        ))}
